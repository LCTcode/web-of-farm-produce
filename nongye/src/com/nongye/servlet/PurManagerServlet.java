 ...
 * 这是采购管理的servlet
 */
@WebServlet("/purManagerServlet")
public class PurManagerServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    
    public PurManagerServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(strPageNo==null){//如果为空则没接收到页面参数
			...
			curPageNo=Integer.parseInt(request.getParameter("curPageNo"));
				...
				try {
					//获取总记录数
					totalSize = PPN.PageNo_1(user_id);
					//获取总页面数
					int totalPageSize=PPN.pagesTotal_1(everyPageSize_1,user_id);
					//保存总记录与总页数
		...
		List<Purchase> list = GDa.selectPurManager(curPageNo,everyPageSize_1,user_id);
