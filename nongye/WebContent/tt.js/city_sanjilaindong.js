var addressInit = function(_province, _city, _area, _UserStreet, defaultProvince, defaultCity, defaultArea,defaultStreet)   //不是很理解这里传递参数为什么有“_”??? ，后来想到这样的书写意义是什么，后来琢磨一下，浏览器的执行是书写先后执行，我们将js外部引用，在开头，先执行此js ，此时dom要获取的id还没有创建，这样写是为了区分开待会传的id 值，执行顺序：外部js html 执行addressinnit()方法， 此方法已经在js 中创建，此时回头执行，就跟onclick="addressInnit()"一样，只是这里不需要点击，我们将参数的“_”去掉也没事，但是却在传值上可能产生误会。
{ 
...
var UserStreet = document.getElementById(_UserStreet);    
...
{ 
UserAddOption(area, item.areaList[i], null); 
} 
UserSelect(area, defaultArea); 
} 
function changeProvince()     
{ 
city.options.length = 0; 
city.onchange = null; 
if(province.selectedIndex == -1)return;    //判断我们是否在第一个栏已经有选择，
var item = province.options[province.selectedIndex].obj;  //获取我们在第一个栏的选择的值。
for(var i=0; i<item.cityList.length; i++) 
{ 
UserAddOption(city, item.cityList[i].name, item.cityList[i]); // 动态创建第二个栏，也就是城市栏的option 及其value。
} 
UserSelect(city, defaultCity);  //判断我们在city 是否选择，，
changeCity(); 
...
{ 
UserAddOption(province, provinceList[i].name, provinceList[i]); //看到这里就虚服了，执行这个方法，动态创建option 节点，根据传的id值判断在那里创建，之后显示name，，并显示其value 执行 i 次，    
} 
UserSelect(province, defaultProvince);  //执行select方法，此方法作用是？我觉得是判断选择，当我们选择时，看上面的方法，selctIndex=1；此时在changeprovice中就会又一个判断，若=-1 return，否则执行以下，也就是执行完changeprovice.
changeProvince(); 
